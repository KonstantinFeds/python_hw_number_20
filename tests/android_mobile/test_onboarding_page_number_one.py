...
def test_add_language():
    ...
    browser.element(
        (AppiumBy.ANDROID_UIAUTOMATOR, 'className("android.widget.Button").instance(1)')
    ).click()
    browser.element(
        (AppiumBy.ANDROID_UIAUTOMATOR, 'text("Search for a language")')
    ).click()
    ...
    browser.all((AppiumBy.ID, "org.wikipedia.alpha:id/option_label"))[1].should(
        have.text("Русский")
    )
